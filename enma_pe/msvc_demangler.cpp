...
/*
    https://github.com/wine-mirror/wine/blob/07cf14dc928a1a00baecbbc7ca5a6f3fe680238c/dlls/msvcrt/undname.c
*/
...
struct alloc_array
{
    unsigned            start;          /* first valid reference in alloc_array */
...
struct parsed_symbol
{
    ...
    alloc_array        names;          /* alloc_array of names for back reference */
    alloc_array        stack;          /* stack of parsed strings */
...
/******************************************************************
*		str_alloc_array_init
* Initialises an alloc_array of strings
*/
static void str_alloc_array_init(struct alloc_array* a)
{
...
/******************************************************************
*		str_alloc_array_push
* Adding a new string to an alloc_array
*/
static bool str_array_push(struct parsed_symbol* sym, const char* ptr, int len,
    struct alloc_array* a)
{
    char**      __new;
    ...
    if (!a->alloc)
    {
        __new = (char**)und_alloc(sym, (a->alloc = 32) * sizeof(a->elts[0]));
        if (!__new) return false;
        a->elts = __new;
    }
    else if (a->max >= a->alloc)
    {
        __new = (char**)und_alloc(sym, (a->alloc * 2) * sizeof(a->elts[0]));
        if (!__new) return false;
        memcpy(__new, a->elts, a->alloc * sizeof(a->elts[0]));
        a->alloc *= 2;
        a->elts = __new;
    }
...
/******************************************************************
*		str_alloc_array_get_ref
* Extracts a reference from an existing alloc_array (doing proper type
* checking)
*/
static char* str_alloc_array_get_ref(struct alloc_array* cref, unsigned idx)
{
    assert(cref);
...
static bool demangle_datatype(struct parsed_symbol* sym, struct datatype_t* ct,
    struct alloc_array* pmt, bool in_args);
...
*/
static char* get_args(struct parsed_symbol* sym, struct alloc_array* pmt_ref, bool z_term,
    char open_char, char close_char)

{
    struct datatype_t   ct;
    struct alloc_array        arg_collect;
    char*               args_str = NULL;
    char*               last;
    unsigned int        i;

    str_alloc_array_init(&arg_collect);
...
static bool get_modified_type(struct datatype_t *ct, struct parsed_symbol* sym,
    struct alloc_array *pmt_ref, char modif, bool in_args)
{
    const char* modifier;
...
static char* get_literal_string(struct parsed_symbol* sym)
{
    ...
    return str_alloc_array_get_ref(&sym->names, sym->names.num - sym->names.start - 1);
}
...
* name with the template arguments.  We use the same stack alloc_array
...
static char* get_template_name(struct parsed_symbol* sym)
{
    ...
    struct alloc_array alloc_array_pmt;
    ...
    str_alloc_array_init(&alloc_array_pmt);
    args = get_args(sym, &alloc_array_pmt, false, '<', '>');
    if (args != NULL)
...
* result in 'a' alloc_array. Each parent-classes, as well as the inner element
...
static bool get_class(struct parsed_symbol* sym)
{
    ...
    while (*sym->current != '@')
    {
        ...
        case '8': case '9':
            name = str_alloc_array_get_ref(&sym->names, *sym->current++ - '0');
            break;
        case '?':
            switch (*++sym->current)
            {
            ...
            case '?':
            {
                struct alloc_array stack = sym->stack;
                unsigned int start = sym->names.start;
                unsigned int num = sym->names.num;

                str_alloc_array_init(&sym->stack);
...
/******************************************************************
*		get_class_string
* From an alloc_array collected by get_class in sym->stack, constructs the
...
static char* get_class_string(struct parsed_symbol* sym, int start)
{
    ...
    unsigned int len, sz;
    char*        ret;
    struct alloc_array *a = &sym->stack;
...
*/
static bool demangle_datatype(struct parsed_symbol* sym, struct datatype_t* ct,
    struct alloc_array* pmt_ref, bool in_args)
{
    char                dt;
    bool                add_pmt = true;
            ...
            *   others who knows.. */
            if (*sym->current == '8')
            {
                ...
                ct->left = str_printf(sym, "%s%s (%s %s::*",
                    sub_ct.left, sub_ct.right, call_conv, __class);
    ...
    case '5': case '6': case '7': case '8': case '9':
        ...
        ct->left = str_alloc_array_get_ref(pmt_ref, (dt - '0') * 2);
        ct->right = str_alloc_array_get_ref(pmt_ref, (dt - '0') * 2 + 1);
...
static bool handle_data(struct parsed_symbol* sym)
{
    ...
    */
    ...
    {
        unsigned mark = sym->stack.num;
        struct alloc_array pmt;

        str_alloc_array_init(&pmt);
...
static bool handle_method(struct parsed_symbol* sym, bool cast_op)
{
    ...
    */
    ...
    {
        if (!demangle_datatype(sym, &ct_ret, &alloc_array_pmt, false))
            goto done;
    }
    ...
    mark = sym->stack.num;
    if (has_args && !(args_str = get_args(sym, &alloc_array_pmt, true, '(', ')'))) goto done;
    ...
    if (*sym->current == '?' && (sym->current[1] != '$' || sym->current[2] == '?'))
    {
        ...
        case '_':
            ...
            case 'R':
                ...
                break;
                case '2': function_name = "`RTTI Base Class alloc_array'"; break;
    ...
    void* unknown, unsigned short int flags)
{
    ...
    */
    ...
    sym.flags = flags;
    sym.current = mangled;
    str_alloc_array_init(&sym.names);
    str_alloc_array_init(&sym.stack);
    ...
    {
        buffer = new char[(strlen(result) + 1)];
